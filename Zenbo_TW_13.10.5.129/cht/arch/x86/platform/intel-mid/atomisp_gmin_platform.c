...
static struct gmin_subdev *gmin_subdev_add(struct v4l2_subdev *subdev)
{
	...
	struct device *dev;
	struct i2c_client *client = v4l2_get_subdevdata(subdev);
	const char * dev_name_t4k37 = "i2c-INT33BA:00";
	...
	if (strcmp(dev_name(dev), dev_name_t4k37) == 0) {
		pr_info("%s, in t4k37 config\n", __func__);
		gmin_subdevs[i].clock_num = gmin_get_var_int(dev, "CamClk", 0);
		gmin_subdevs[i].clock_src = gmin_get_var_int(dev, "ClkSrc", VLV2_CLK_XTAL_19P2MHZ);
...
int gmin_v2p8_vcm_ctrl(struct v4l2_subdev *subdev, int on)
{
	struct gmin_subdev *gs = find_gmin_subdev(subdev);
	int ret = 0;

	if (gs && gs->v2p8_vcm_on == on)
		return 0;
	gs->v2p8_vcm_on = on;

	if (gs->v2p8_vcm_reg) {
		pr_debug("regulator v2p8_vcm on=%d\n", on);
		if (on) {
			ret = regulator_set_voltage(gs->v2p8_vcm_reg, 2800000, 2800000);
			if (ret)
	...
	/* TODO: v2p8_vcm needs to extend to other PMICs */
...
static struct camera_sensor_platform_data gmin_plat = {
	...
	.v2p8_vcm_ctrl = gmin_v2p8_vcm_ctrl,
