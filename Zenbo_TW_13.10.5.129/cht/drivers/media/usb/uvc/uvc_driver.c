...
static struct wake_lock R200_wakelock;
static int R200_register_cnt = 0;
...
static void uvc_unregister_video(struct uvc_device *dev)
{
	 ...
	 * are no stream left.
	 */
	if (!strcmp(dev->udev->product, "Intel RealSense 3D Camera LR200")) {
		if(--R200_register_cnt == 0) {
			pr_info("R200 wake_unlock\n");
			wake_unlock(&R200_wakelock);
			wake_lock_destroy(&R200_wakelock);
		}
	}
		...
		struct uvc_streaming *stream)
{
	...
	atomic_inc(&dev->nstreams);
	if (!strcmp(dev->udev->product, "Intel RealSense 3D Camera LR200")) {
		if(++R200_register_cnt == 1) {
			pr_info("R200 wake_lock\n");
			wake_lock_init(&R200_wakelock, WAKE_LOCK_SUSPEND, "R200_wake_lock");
			wake_lock(&R200_wakelock);
		}
	}
...
static int __init uvc_init(void)
{
	...
	pr_info("%s\n", __func__);
	if (check_cos_mode()) {
		pr_info("Don't execute UVC driver init under COS mode\n");
		return -ENODEV;
	}
