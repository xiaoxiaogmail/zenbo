...
#define NVM_TABLE_VERSION	0x5FC5
#define NVM_CUSTOMER_VERSION	0x5FC6
...
static struct dentry *pmic_debugfs_dir_entry_root;
static u8 pmic_status;
static int nvm_table_version;
static int nvm_customer_version;
...
#ifdef CONFIG_DEBUG_FS
	pmic_debugfs_dir_entry_root = debugfs_create_dir("pmic", NULL);
	if (pmic_debugfs_dir_entry_root) {
		nvm_table_version = intel_soc_pmic_readb(NVM_TABLE_VERSION);
		nvm_customer_version = intel_soc_pmic_readb(NVM_CUSTOMER_VERSION);
		if (nvm_table_version < 0 || nvm_customer_version < 0) {
			pmic_status = 0;
		} else {
			pmic_status = 1;
			debugfs_create_x8("nvm_table_version", S_IRUGO, pmic_debugfs_dir_entry_root, &nvm_table_version);
			debugfs_create_x8("nvm_customer_version", S_IRUGO, pmic_debugfs_dir_entry_root, &nvm_customer_version);
		}
		debugfs_create_u8("pmic_status", S_IRUGO, pmic_debugfs_dir_entry_root, &pmic_status);
	}
#endif /* CONFIG_DEBUG_FS */
