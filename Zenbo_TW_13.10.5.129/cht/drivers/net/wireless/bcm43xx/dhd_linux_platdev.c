...
int
write_mac_to_file(uint8 *buf, int size)
{
        int ret = 0;
        struct file *fp = NULL;
        mm_segment_t old_fs;
        ...
        DHD_ERROR(("%s: ranmac_path = %s\n", __FUNCTION__, WIFI_RAN_MAC_ADDR_FILE));

        /* open file to write */
        fp = filp_open(WIFI_RAN_MAC_ADDR_FILE, file_mode, 0644);
        if (IS_ERR(fp)) {
...
static int wifi_get_external_mac_addr(unsigned char *buf)
{
	...
	if (fp == NULL) {
		...
		fp = dhd_os_open_image(WIFI_RAN_MAC_ADDR_FILE);
		rand = 1;
		if (fp == NULL) {
	...
	if (rand)
		DHD_ERROR(("%s: read from file mac address: %x:%x:%x:%x:%x:%x, from %s\n",
		   __func__, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], WIFI_RAN_MAC_ADDR_FILE));
	else
		DHD_ERROR(("%s: read from file mac address: %x:%x:%x:%x:%x:%x, from %s\n",
		   __func__, buf[0], buf[1], buf[2], buf[3], buf[4], buf[5], fname));
...
random_mac:
	...
	if (write_mac_to_file(str, 12) < 0)
		pr_err("%s: mac write failed.\n", __func__);
	else
		pr_err("%s: mac write successed.\n", __func__);
