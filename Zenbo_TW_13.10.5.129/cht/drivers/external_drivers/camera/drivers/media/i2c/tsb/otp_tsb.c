...
#define TSB_OTP_PAGE_SIZE	32
#define TSB_OTP_DATA_SIZE	512
#define TSB_OTP_READ_ONETIME	32
...
static int __tsb_otp_read(struct v4l2_subdev *sd, u8 *read_value)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;
	int i;

	ret = tsb_write_reg(client, TSB_8BIT, TSB_OTP_ENABLE, 0x0);
	if (ret) {
		dev_err(&client->dev, "failed to write otp enable (0x0).\n");
		return ret;
	}
	...
	for (i = 2; i >= 0; i--) {
		...
		if (ret) {
			dev_err(&client->dev, "failed to write otp enable (0x81).\n");
			return ret;
		}
		...
		if (ret) {
			dev_err(&client->dev, "failed to read OTP data (0x0)\n");
			return ret;
		}

		pr_info("%s Check bank %d 0x%X 0x%X\n", __func__, i, read_value[0], read_value[1]);
...
int tsb_otp_read(struct v4l2_subdev *sd, u8 *otp_data)
{
	struct i2c_client *client = v4l2_get_subdevdata(sd);
	int ret;

	ret = __tsb_otp_read(sd, otp_data);
