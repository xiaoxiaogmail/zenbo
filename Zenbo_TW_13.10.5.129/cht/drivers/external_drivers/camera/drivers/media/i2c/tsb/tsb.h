...
struct tsb_otp {
	int (*otp_read)(struct v4l2_subdev *sd, u8 *otp_data);
	u32 start_addr;
	u32 size;
	u8 dev_addr;
};

struct tsb_otp_struct {
	u8 af_inf[2];
	u8 af_30cm[2];
	u8 af_8cm[2];
	u8 start_current[2];
...
extern int tsb_otp_read(struct v4l2_subdev *sd, u8 *otp_data);
