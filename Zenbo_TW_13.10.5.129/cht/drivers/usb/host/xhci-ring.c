...
void send_r200_tx_error_uevent(struct work_struct *work)
{
	struct send_uevent_struct *su_struct = container_of(work, struct send_uevent_struct, work);
	struct usb_device *udev = su_struct->udev;
	char *uevent_envp[] = { "R200_STATE=HALTED", NULL};

	kobject_uevent_env(&udev->dev.kobj, KOBJ_CHANGE, uevent_envp);
	printk("xhci_hcd: send_r200_tx_error_uevent: R200_STATE=HALTED");
}
		...
		if (ret) {
			...
			if ((urb->actual_length != urb->transfer_buffer_length &&
				...
				xhci_dbg(xhci, "Giveback URB %p, len = %d, "
						...
						status);
				udev = urb->dev;
				if (udev->product && status == -EPROTO && !(xhci->uevent_sended)) {
					if (!strcmp(udev->product, "Intel RealSense 3D Camera LR200")) {
						xhci->uevent_sended = true;
						xhci->su_struct->udev = udev;
						queue_work(xhci->send_uevent_wq, &(xhci->su_struct->work));
					}
				}
			}
			spin_unlock(&xhci->lock);
