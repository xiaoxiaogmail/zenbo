...
int xhci_check_bandwidth(struct usb_hcd *hcd, struct usb_device *udev)
{
	...
	if (udev->product && xhci->uevent_sended) {
		if (!strcmp(udev->product, "Intel RealSense 3D Camera LR200")) {
			xhci->uevent_sended = false;
		}
	}
...
int xhci_gen_setup(struct usb_hcd *hcd, xhci_get_quirks_t get_quirks)
{
	...
	int			retval;
	struct send_uevent_struct *su_struct;
	...
	su_struct = kzalloc(sizeof(*su_struct), GFP_KERNEL);
        if (su_struct == NULL)
                xhci_err(xhci, "Alloc GFP_KERNEL memory of send_uevent_struct failed.");
	xhci->su_struct = su_struct;

	INIT_WORK(&su_struct->work, send_r200_tx_error_uevent);

	xhci->send_uevent_wq = create_singlethread_workqueue("send_uevent_wq");
        if (!xhci->send_uevent_wq)
        {
                xhci_err(xhci, "Creat workqueue send_uevent_wq failed.");
        }
