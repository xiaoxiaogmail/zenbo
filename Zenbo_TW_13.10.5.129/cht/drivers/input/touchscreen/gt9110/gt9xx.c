...
static const unsigned short normal_i2c[] = { 0xBA, I2C_CLIENT_END };
...
static s8 gtp_re_request_irq(struct goodix_ts_data *ts);
static void gtp_circle(struct goodix_ts_data* ts,s32 id,s32 input_x,s32 input_y,s32 input_w);
...
static int gtp_power_hal_suspend_init(struct device *dev);
static void gtp_power_hal_suspend_destroy(struct device *dev);
	...
	if (pre_touch || touch_num)
	{
		...
		for (i = 0; i < GTP_MAX_TOUCH; i++)
		{
			...
			if ((touch_index & (0x01<<i)))
			{
				...
				input_x = GTP_MAX_HEIGHT - input_x - 1;
#endif
#if GTP_CIRCLE_ENABLE
				gtp_circle(ts, id, input_x, input_y, input_w);
#else
...
static void gtp_circle(struct goodix_ts_data* ts,s32 id,s32 input_x,s32 input_y,s32 input_w)
{
	if(input_x >= GTP_DISTANCE_EDGE && input_x <= GTP_DISTANCE_SQUARE){
		if(input_y >= GTP_DISTANCE_EDGE && input_y <= GTP_DISTANCE_SQUARE){
			int dis = (input_x - GTP_CIRCLE_ONE_X) * (input_x - GTP_CIRCLE_ONE_X) + (input_y - GTP_CIRCLE_ONE_Y) * (input_y - GTP_CIRCLE_ONE_Y);
			if(dis <= GTP_CIRCLE_RADIUS_SQUARE){
				GTP_DEBUG("Location ONE: GTP_CIRCLE_ONE_X = %d, GTP_CIRCLE_ONE_Y = %d, dis = %d, input_x = %d, input_y = %d",GTP_CIRCLE_ONE_X, GTP_CIRCLE_ONE_Y, dis, input_x, input_y);
				gtp_touch_down(ts, id, input_x, input_y, input_w);
			} else {
				GTP_INFO("Location ONE: <IGNORE> input_x = %d, input_y = %d", input_x, input_y);
			}
		}//Location ONE

		if(input_y > GTP_DISTANCE_SQUARE && input_y < (ts->abs_y_max - GTP_DISTANCE_SQUARE)){
			GTP_DEBUG("Location FOUR:input_x = %d, input_y = %d", input_x, input_y);
			gtp_touch_down(ts, id, input_x, input_y, input_w);
		}//Location FOUR

		if(input_y >= (ts->abs_y_max - GTP_DISTANCE_SQUARE) && input_y <= (ts->abs_y_max - GTP_DISTANCE_EDGE)){
			int dis = (input_x - GTP_CIRCLE_THREE_X) * (input_x - GTP_CIRCLE_THREE_X) + (input_y - GTP_CIRCLE_THREE_Y) * (input_y - GTP_CIRCLE_THREE_Y);
			if(dis <= GTP_CIRCLE_RADIUS_SQUARE){
				GTP_DEBUG("Location SIX: GTP_CIRCLE_THREE_X = %d, GTP_CIRCLE_THREE_Y = %d, dis = %d, input_x = %d, input_y = %d", GTP_CIRCLE_THREE_X, GTP_CIRCLE_THREE_Y, dis, input_x, input_y);
				gtp_touch_down(ts, id, input_x, input_y, input_w);
			} else {
				GTP_INFO("Location SIX: <IGNORE> input_x = %d, input_y = %d", input_x, input_y);
			}
		}//Location SIX
	}

	if(input_x > GTP_DISTANCE_SQUARE && input_x < (ts->abs_x_max - GTP_DISTANCE_SQUARE)){
		if(input_y > GTP_DISTANCE_EDGE && input_y < (ts->abs_y_max - GTP_DISTANCE_EDGE)){
			GTP_DEBUG("Location TWO: input_x = %d, input_y = %d", input_x, input_y);
			gtp_touch_down(ts, id, input_x, input_y, input_w);
		}//Location TWO
	}

	if(input_x >= (ts->abs_x_max - GTP_DISTANCE_SQUARE) && input_x <= (ts->abs_x_max - GTP_DISTANCE_EDGE)){
		if(input_y >= GTP_DISTANCE_EDGE && input_y <= GTP_DISTANCE_SQUARE){
			int dis = (input_x - GTP_CIRCLE_TWO_X) * (input_x - GTP_CIRCLE_TWO_X) + (input_y - GTP_CIRCLE_TWO_Y) * (input_y - GTP_CIRCLE_TWO_Y);
			if(dis <= GTP_CIRCLE_RADIUS_SQUARE){
				GTP_DEBUG("Location THREE: GTP_CIRCLE_TWO_X = %d, GTP_CIRCLE_TWO_Y = %d, dis = %d, input_x = %d, input_y = %d", GTP_CIRCLE_TWO_X, GTP_CIRCLE_TWO_Y, dis, input_x, input_y);
				gtp_touch_down(ts, id, input_x, input_y, input_w);
			} else {
				GTP_INFO("Location THREE: <IGNORE> input_x = %d, input_y = %d", input_x, input_y);
			}
		}//Location THREE

		if(input_y > GTP_DISTANCE_SQUARE && input_y < (ts->abs_y_max - GTP_DISTANCE_SQUARE)){
			GTP_DEBUG("Location FIVE: input_x = %d, input_y = %d", input_x, input_y);
			gtp_touch_down(ts, id, input_x, input_y, input_w);
		}//Location FIVE

		if(input_y >= (ts->abs_y_max - GTP_DISTANCE_SQUARE) && input_y <= (ts->abs_y_max - GTP_DISTANCE_EDGE)){
			int dis = (input_x - GTP_CIRCLE_FOUR_X) * (input_x - GTP_CIRCLE_FOUR_X) + (input_y - GTP_CIRCLE_FOUR_Y) * (input_y - GTP_CIRCLE_FOUR_Y);
			if(dis <= GTP_CIRCLE_RADIUS_SQUARE){
				GTP_DEBUG("Location SEVEN: GTP_CIRCLE_FOUR_X = %d, GTP_CIRCLE_FOUR_Y = %d, dis = %d, input_x = %d, input_y = %d", GTP_CIRCLE_FOUR_X, GTP_CIRCLE_FOUR_Y, dis, input_x, input_y);
				gtp_touch_down(ts, id, input_x, input_y, input_w);
			} else {
				GTP_INFO("Location SEVEN: <IGNORE> input_x = %d, input_y = %d", input_x, input_y);
			}
		}//Location SEVEN
	}
}
...
ssize_t get_touch_status(struct device *dev, struct device_attribute *attr, char *buf)
{
	struct goodix_ts_data *ts = i2c_get_clientdata(i2c_connect_client);
	s32 ret = -1;
	u8 status = ts->gtp_status;

	if (status == 1 && !ts->gtp_is_suspend) {
		ret = gtp_read_version(i2c_connect_client, &ts->version);
		if (ret < 0) {
	...
	GTP_INFO("touch status = %d", status);
...
static s8 gtp_request_io_port(struct goodix_ts_data *ts)
{
	...
	/* Add SW81 to enable touch power */
	ret = GTP_GPIO_REQUEST(GTP_EN_POWER, "SOC_TP_EN");
	if (ret < 0) {
		GTP_ERROR("Failed to request GPIO:%d, ERRNO:%d", GTP_EN_POWER, ret);
		ret = -ENODEV;
		goto request_en_failed;
	}
	GTP_GPIO_OUTPUT(GTP_EN_POWER, 1);
...
static s8 gtp_re_request_irq(struct goodix_ts_data *ts)
{
	...
	if (!ts->use_irq) {
		GTP_DEBUG("gtp_re_request_irq: no use irq");
		return ret;
	}
	...
	ret  = request_irq(ts->client->irq,
					   ...
					   ts);
	if (ret < 0) {
		GTP_ERROR("re-request irq error %d\n", ret);
		return ret;
	}
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	GTP_INFO("use gpio defined in gt9xx.h");
	...
	ret = gtp_read_version(client, &ts->version);
	if (ret < 0)
	{
		GTP_ERROR("Read version failed.");
	}
	...
	ret = gtp_power_hal_suspend_init(&client->dev);
	if (ret < 0)
...
static int goodix_ts_remove(struct i2c_client *client)
{
	...
	gtp_power_hal_suspend_destroy(&ts->client->dev);
...
static void gtp_power_hal_suspend(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct goodix_ts_data *ts = i2c_get_clientdata(client);

	if (ts) {
		GTP_DEBUG("Suspend by power HAL");
		goodix_ts_suspend(ts);
	}
}

static void gtp_power_hal_resume(struct device *dev)
{
	struct i2c_client *client = to_i2c_client(dev);
	struct goodix_ts_data *ts = i2c_get_clientdata(client);

	if (ts) {
		GTP_DEBUG("Resume by power HAL");
		goodix_ts_resume(ts);
	}
}

static ssize_t gtp_power_hal_suspend_store(struct device *dev,
						 ...
						 const char *buf, size_t count)
{
	...
	if (!strncmp(buf, POWER_HAL_SUSPEND_ON,
		...
		if (!ts->gtp_is_suspend)
			gtp_power_hal_suspend(dev);
	} else {
		if (ts->gtp_is_suspend)
			gtp_power_hal_resume(dev);
	}

	return count;
}
static DEVICE_POWER_HAL_SUSPEND_ATTR(gtp_power_hal_suspend_store);
...
// CONFIG_FB and CONFIG_HAS_EARLYSUSPEND cannot be supported on this platform.
#if 0
...
static const struct acpi_device_id goodix_acpi_table[] = {
	{ "GDI9110", 0 },
	{ }
};
...
static struct i2c_driver goodix_ts_driver = {
	...
	.driver = {
		.name	 = GTP_I2C_NAME,
		.owner	= THIS_MODULE,
		.acpi_match_table = goodix_acpi_table,
#ifdef GTP_CONFIG_OF
...
static int goodix_ts_init(void)
{
	...
	if (asustek_get_hw_rev() < 1) {//SR2
		GTP_INFO("No goodix touch IC");
		return -ENODEV;
	}
