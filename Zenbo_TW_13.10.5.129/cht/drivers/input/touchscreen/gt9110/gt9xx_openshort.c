...
extern ssize_t get_touch_status(struct device *dev, struct device_attribute *attr, char *buf);
extern ssize_t get_fw_version(struct device *dev, struct device_attribute *attr, char *buf);
...
s32 gtp_parse_config(struct i2c_client *client)
{
	...
	{
		GTP_INFO("No key");
		gt9xx_sc_drv_num = gt9xx_drv_num; //add by one, provide  gt9xx_sc_drv_num value
		return SUCCESS;
	}
	else
	{
...
load_dsp_again:
	...
	*/
	...
	if ((opr_buf[2] & 0x0f) == 0)
	{
		SET_INFO_LINE_INFO("[TEST SUCCEEDED]");
		ret = SUCCESS;
		//add by one
...
static void gtp_uniformity_test(u16 *raw_buf)
{
	...
	min_val = raw_buf[0];
	max_val = raw_buf[0];
	//for (i = 1; i < gt9xx_sc_pxl_cnt; i++)	//modify by one, i should be start zero
	for (i = 0; i < gt9xx_sc_pxl_cnt; i++)
	{
...
{
	u16 i = 0, j = 0, ds_val, len;
	u16 min_val = 0, max_val = 0;
	u32 avg_val = 0;
	char tmp_buffer[6+1];
	char print_buffer[MAX_SENSOR_NUM*6+1];

	GTP_INFO("Total node number: %d", gt9xx_sc_pxl_cnt);
...
#if PRINT_RAW_DATA
		len = 0;
		memset(print_buffer, '\0', MAX_SENSOR_NUM*6+1);
#endif
...
#if PRINT_RAW_DATA
			sprintf(tmp_buffer, " %d\0", raw_buf[ds_val]);
			memcpy(&print_buffer[len], tmp_buffer, strlen(tmp_buffer));
	...
	GTP_INFO("Test Limit Rawdata Max:%d, Min:%d", test_parameter[gtp_test_sensor_id].max_limit_value, test_parameter[gtp_test_sensor_id].min_limit_value);
	GTP_INFO("Last Frame Rawdata Max:%d, Min:%d, Avg:%d", max_val, min_val, (avg_val / gt9xx_sc_pxl_cnt));
	SET_INFO_LINE_INFO("Test Limit Rawdata Max:%d, Min:%d", test_parameter[gtp_test_sensor_id].max_limit_value, test_parameter[gtp_test_sensor_id].min_limit_value);
...
static s32 gtp_get_test_result(void)
{
	...
	if (beyond_uniformity > _MIN_ERROR_NUM)
	{
		...
		SET_INFO_LINE_INFO("  Uniformity Limit: %d%%, Tp Uniformity: %d%%", test_parameter[gtp_test_sensor_id].uniformity_lmt, touchpad_sum[4*_BEYOND_INFO_MAX].raw_val / touchpad_sum[4 * _BEYOND_INFO_MAX].times);
	}
	...
	{
		SET_INFO_LINE_INFO("  Beyond Raw Max Limit [Max Limit: %d]", test_parameter[gtp_test_sensor_id].max_limit_value);
	}
	if (result & _BEYOND_MIN_LIMIT)
	{
		SET_INFO_LINE_INFO("  Beyond Raw Min Limit [Min Limit: %d]", test_parameter[gtp_test_sensor_id].min_limit_value);
	}
	...
	if (have_key)
	{
		...
		{
			SET_INFO_LINE_INFO("  Beyond KeyVal Max Limit [Key Max Limit: %d]", test_parameter[gtp_test_sensor_id].max_limit_key);
		}
		if (result & _BEYOND_KEY_MIN_LMT)
		{
			SET_INFO_LINE_INFO("  Beyond KeyVal Min Limit [Key Min Limit: %d]", test_parameter[gtp_test_sensor_id].min_limit_key);
		}
	}

	if (result & _BEYOND_UNIFORMITY_LMT)
	{
		SET_INFO_LINE_INFO("  Beyond Uniformity Limit [Uniformity Limit: %d%%]", test_parameter[gtp_test_sensor_id].uniformity_lmt);
	}
	return result;
}
...
s32 gt9xx_open_test(struct i2c_client * client)
{
	...
	gtp_raw_print(raw_buf); //only printf last raw data infor
 ...
 *
 ===================================================
*/
s8 gt9xx_bincfg_check(struct i2c_client * client)
{
	s8 ret = FAIL; // SUCCESS, FAIL
	struct goodix_ts_data *ts;
	...
	//add by one to read sensor id and set test parameter, and first set ret value
	gtp_test_sensor_id = gtp_get_sensor_id();
	if((gtp_test_sensor_id > 5) || (gtp_test_sensor_id < 0))
	{
		SET_INFO_LINE_ERR("Sensor ID: %d, Check fail due to ID only 0~5.", gtp_test_sensor_id);
		ret = FAIL;
	}
	else
	{
		SET_INFO_LINE_INFO("Sensor ID: %d", gtp_test_sensor_id);
		ret = SUCCESS;
	}
	...
	//if buf[5] = 0, set source and target = "_"
	if(buf[5] == 0x00)
	{
		buf[5] = '_';
		ansbuf[3] = '_';
	...
	//print IC tyep and FW version
	SET_INFO_LINE_INFO("IC_FW Version: %c%c%c%c_%x(def), %c%c%c%c_%x(sor)", ansbuf[0], ansbuf[1], ansbuf[2], ansbuf[3], GTP_BIN_VERSION, buf[2], buf[3], buf[4], buf[5], ((buf[7] << 8) + buf[6]));

	if((ansbuf[0] != buf[2]) || (ansbuf[1] != buf[3]) || (ansbuf[2] != buf[4]) || (ansbuf[3] != buf[5]) || (((buf[7] << 8) + buf[6]) != GTP_BIN_VERSION))
	{
		//add F/W update and check again
		//SET_INFO_LINE_INFO("IC or FW not the same and find BIN file update");
		//FW_update.......
		...
		//direct read and check, if success show pass else show F/W update finish but still not equ default MP F/W version
		//SET_INFO_LINE_INFO("IC or FW not the same and not have BIN file");

		SET_INFO_LINE_ERR("IC or FW check Fail!");
		ret = FAIL;
	}
	else
	{
		SET_INFO_LINE_INFO("IC, FW version check PASS!");
	}

	//check CFG version
	buf[0] = (GTP_REG_CONFIG_DATA >> 8), buf[1] = (GTP_REG_CONFIG_DATA & 0xff);
	gtp_i2c_read(client, buf, sizeof(buf));
	SET_INFO_LINE_INFO("CFG Version: %d(def), %d(sor)", test_parameter[gtp_test_sensor_id].cfg_version, buf[2]);

	if(buf[2]!= test_parameter[gtp_test_sensor_id].cfg_version)
	{
		SET_INFO_LINE_ERR("CFG check Fail!");
		ret = FAIL;
	}
	else
	{
		SET_INFO_LINE_INFO("CFG version check PASS!");
	}
	...
	SET_INFO_LINE_INFO("CFG Checksum(0x80FF) = %x", buf[2]); //printf first byte
	...
	if(buf[2] != test_parameter[gtp_test_sensor_id].cfg_chksum)
	{
		...
		gtp_i2c_read(client, buf, sizeof(buf));
		SET_INFO_LINE_INFO("CFG Checksum(0x8129) = %x", buf[2]); //printf first byte

		if(buf[2]!= test_parameter[gtp_test_sensor_id].cfg_chksum)
		{
			SET_INFO_LINE_ERR("CFG Define Chksum = %02x Fail!", test_parameter[gtp_test_sensor_id].cfg_chksum);
			ret = FAIL;
		}
	}
	else
	{
		SET_INFO_LINE_INFO("CFG Chksum: %02x(def), %02x(sor)", test_parameter[gtp_test_sensor_id].cfg_chksum, buf[2]);
		SET_INFO_LINE_INFO("CFG Chksum check PASS!");
	}
...
s32 gt9xx_rstint_i2c_test(struct i2c_client * client)
{
	...
	for( i = 0; i < 50 ; i ++)
	{
		...
		{
			SET_INFO_LINE_ERR("I2C test ERROR!");
			goto rstint_i2c_test_exit;
		}
	}
	SET_INFO_LINE_INFO("I2C test PASS!");

	//test RST, hold RST can't read I2C information, if read any i2c data then RST isn't connect
	GTP_GPIO_OUTPUT(GTP_RST_PORT, 0);   // rst ic
	msleep(10);
	if(gtp_i2c_test_check(ts->client)) // if success must be error
	{
		SET_INFO_LINE_ERR("RST test ERROR!");
		goto rstint_i2c_test_exit;
	}
	SET_INFO_LINE_INFO("RST test PASS!");
	...
	if(gtp_i2c_test_check(ts->client)) // if success must be error
	{
		SET_INFO_LINE_ERR("INT test ERROR!");
		goto rstint_i2c_test_exit;
	}
	SET_INFO_LINE_INFO("INT test PASS!");
	gtp_reset_guitar(ts->client, 10);
...
static ssize_t gtp_sysfs_bincfg_show(struct device *dev,struct device_attribute *attr, char *buf)
{
	gtp_test_sensor_id = gtp_get_sensor_id();

	return sprintf(buf, "max_limit=%d, min_limit=%d, uniformity=%d\n",
		test_parameter[gtp_test_sensor_id].max_limit_value,
		test_parameter[gtp_test_sensor_id].min_limit_value,
		test_parameter[gtp_test_sensor_id].uniformity_lmt);
}

static ssize_t gtp_sysfs_bincfg_store(struct device *dev,struct device_attribute *attr, const char *buf, size_t count)
{
	int num, max_limit, min_limit, uniformity;

	gtp_test_sensor_id = gtp_get_sensor_id();

	num = sscanf(buf, "%d,%d,%d", &max_limit, &min_limit, &uniformity);
	if (num == 3) {
		if (min_limit < 500 || max_limit < min_limit || uniformity < 60) {
			GTP_ERROR("parameters are not valid\n");
			return -EINVAL;
		}
		GTP_INFO("set max_limit=%d, min_limit=%d, uniformity=%d to bincfg\n",
				max_limit, min_limit, uniformity);
		test_parameter[gtp_test_sensor_id].max_limit_value = max_limit;
		test_parameter[gtp_test_sensor_id].min_limit_value = min_limit;
		test_parameter[gtp_test_sensor_id].uniformity_lmt = uniformity;
	} else {
		GTP_ERROR("parsing bincfg failed\n");
		return -EINVAL;
	}
	return count;
}
...
static DEVICE_ATTR(bincfg, S_IRUGO|S_IWUSR, gtp_sysfs_bincfg_show, gtp_sysfs_bincfg_store);
...
s32 gtp_test_sysfs_init(void)
{
	...
	ret = sysfs_create_file(goodix_debug_kobj, &dev_attr_bincfg.attr);
	if (ret) {
		GTP_ERROR("%s: sysfs_create_version_file failed\n", __func__);
		return ret;
	}
...
void gtp_test_sysfs_deinit(void)
{
	...
	sysfs_remove_file(goodix_debug_kobj, &dev_attr_bincfg.attr);
