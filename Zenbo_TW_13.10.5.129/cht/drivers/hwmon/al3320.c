...
#define ALS_INT_N 339

int count_div_lux = 128;
int set_mode_first_time = 0;
...
static int al3320_get_int_enable(struct i2c_client *client)
{
	return __al3320_read_reg(client, AL3320_INT_ENABLE,
				 AL3320_INT_ENABLE_MASK,
				 AL3320_INT_ENABLE_SHIFT);
}
...
{
	return __al3320_read_reg(client, AL3320_INT_ENABLE,
				 AL3320_SUS_ENABLE_MASK,
				 AL3320_SUS_ENABLE_SHIFT);
}
#endif
...
static int al3320_get_adc_value_original(struct i2c_client *client, int lock)
{
	struct al3320_data *data = i2c_get_clientdata(client);
	unsigned int lsb, msb;
	unsigned long tmp, range;
...
static int al3320_get_adc_value_lsb(struct i2c_client *client, int lock)
{
	struct al3320_data *data = i2c_get_clientdata(client);
	signed int lsb;
				 ...
				 size_t count)
{
	...
	if (val == 1) {
		al3320_set_althres(data->client, als_high_threshold);
		al3320_set_ahthres(data->client, als_low_threshold);
		printk("=== al3320_set_mode %lu ===", val);
...
/* adc */
static ssize_t al3320_show_adc(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct al3320_data *data = al3320_data_g;
	if (al3320_get_mode(data->client) == 0x00) {
	...
	return sprintf(buf, "%d\n", al3320_get_adc_value_original(data->client, 0));
}

/* adc */
static ssize_t al3320_show_monitor(struct device *dev,
				struct device_attribute *attr, char *buf)
{
	struct al3320_data *data = al3320_data_g;
	if (al3320_get_mode(data->client) == 0x00) {
	...
	return sprintf(buf, "%d\n", al3320_get_adc_value_lsb(data->client, 0));
}
...
static struct device_attribute attributes[] = {
	...
	__ATTR(adc, S_IRUGO, al3320_show_adc, NULL),
	__ATTR(monitor, S_IRUGO, al3320_show_monitor, NULL),
...
static int al3320_init_client(struct i2c_client *client)
{
	...
	if (als_polling)
		...
		al3320_set_althres(client, als_high_threshold);
		al3320_set_ahthres(client, als_low_threshold);
		mdelay(1);
...
static void al3320_work_func_light(struct work_struct *work)
{
	struct al3320_data *data =
	    container_of(work, struct al3320_data, work_light);
	int Aval;

	mutex_lock(&data->lock);

	Aval = al3320_get_adc_value(data->client, 1);
	/* printk(KERN_DEBUG "ALS lux value: %u\n", Aval); */
	mutex_unlock(&data->lock);
	if (set_mode_first_time == 1) {
	...
	al3320_set_althres(data->client, (int)(Aval * 98 * 1000 / count_div_lux / cali));
	al3320_set_ahthres(data->client, (int)(Aval * 102 * 1000 / count_div_lux / cali));
...
{
	struct al3320_data *data =
	    container_of(timer, struct al3320_data, light_timer);
...
static void al3320_timer_init(struct al3320_data *data)
{
	if (als_polling) {
		...
		hrtimer_init(&data->light_timer, CLOCK_MONOTONIC,
			     HRTIMER_MODE_REL);
		data->light_poll_delay =
		    ns_to_ktime(als_poll_delay * NSEC_PER_MSEC);
...
static int create_sysfs_interfaces(void)
{
	...
	al3320_dev = device_create(al3320_class, NULL, 0, "%s", "di_sensors");
...
static irqreturn_t al3320_irq(int irq, void *dev_id)
{
	struct al3320_data *data = dev_id;

	/* mutex_lock(&data->lock); */
	unsigned long flags;
	spin_lock_irqsave(&data->irq_enabled_lock, flags);
	...
	spin_unlock_irqrestore(&data->irq_enabled_lock, flags);
	/* mutex_unlock(&data->lock); */
...
{
	struct al3320_data *data =
	    container_of(h, struct al3320_data, early_suspend);
			...
			const struct i2c_device_id *id)
{
	...
	if (!als_polling) {
		...
		client->irq = gpio_to_irq(ALS_INT_N);
		data->irq = client->irq;
		err = request_irq(client->irq, al3320_irq,
	...
	spin_lock_init(&data->irq_enabled_lock);
...
#ifdef CONFIG_ACPI
static struct acpi_device_id al3320_acpi_match[] = {
	{"ALS3320", 0},
	{"", 0},
};

MODULE_DEVICE_TABLE(acpi, al3320_acpi_match);
#endif
...
static const struct dev_pm_ops al3320_pm_ops = {
...
static struct i2c_driver al3320_driver = {
	.driver = {
		   ...
		   .owner = THIS_MODULE,
		   .acpi_match_table = ACPI_PTR(al3320_acpi_match),
#ifndef CONFIG_HAS_EARLYSUSPEND
		   .pm = &al3320_pm_ops,
#endif
		   },
