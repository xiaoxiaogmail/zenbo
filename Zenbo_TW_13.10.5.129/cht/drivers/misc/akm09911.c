	...
	int32_t *rbuf)
{
	...
	if (ready & MAGUC_DATA_READY) {
		input_report_abs(akm->input, ABS_THROTTLE, rbuf[5]);
		input_report_abs(akm->input, ABS_WHEEL, rbuf[6]);
		input_report_abs(akm->input, ABS_GAS,   rbuf[7]);
		input_report_abs(akm->input, ABS_BRAKE, rbuf[8]);
...
static ssize_t akm_mag_power_store(
	struct device *dev, struct device_attribute *attr,
	char const *buf, size_t count)
{
	if (s_akm) {
		if (atomic_cmpxchg(&s_akm->active, 1, 0) == 1)
...
static struct device_attribute akm_compass_attributes[] = {
	...
	__ATTR(power_mag,    0220, NULL, akm_mag_power_store),
#if AKM_DEBUG_IF
...
static struct acpi_device_id akm9911_acpi_match[] = {
	{ "AKM9911", 0 },
	{ "", 0 },
};
	...
	struct i2c_client *client)
{
...
#if 1
	{
		...
		unsigned char buf[8];
		i2c_addr = 0x68; //mpu65xx i2c addr

		buf[0] = 0x6b;
		buf[1] = 0x00;
		err = mpu_i2c_txdata(client, buf, 2);
		if (err < 0)
			return err;
		buf[0] = 0x37;
		buf[1] = 0x12;
...
static int notify_reboot(struct notifier_block *this,
