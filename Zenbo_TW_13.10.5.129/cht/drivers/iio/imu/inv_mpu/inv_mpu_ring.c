					...
					u8 *data, s64 t)
{
	...
	/* if compass enable only && compass data == 0, send nothing */
	if ((c[0] == 0) && (c[1] == 0) && (c[2] == 0)) {
		if (conf->compass_fifo_enable && (!(conf->quaternion_on & conf->dmp_on))
			&& (!conf->gyro_fifo_enable) && (!conf->accl_fifo_enable))
		{
			return 0;
		}
	}
...
struct iio_kfifo *buf = iio_to_kfifo(indio_dev->buffer);

printk("%s\n", __func__);
printk("bytes_per_datum = %d, %d\n",
    indio_dev->buffer->access->get_bytes_per_datum(indio_dev->buffer),
    indio_dev->buffer->bytes_per_datum);
printk("length = %d, %d\n",
    indio_dev->buffer->access->get_length(indio_dev->buffer),
    indio_dev->buffer->length);

printk("kfifo, esize= %d, size = %d\n", kfifo_esize(&(buf->kf)), kfifo_size(&(buf->kf)));
#endif	
