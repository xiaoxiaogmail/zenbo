...
static struct mpu_platform_data gyro_platform_data_second = {
	.int_config  = 0x10,
	.level_shifter = 0,
	.orientation = {0,  1,  0,
			1,  0,  0,
...
static struct i2c_device_id gyro_id[] = {
	{"mpu6500", INV_MPU6500},
};
#endif
...
#define INV_ACCL_CHAN(_type, _channel2, _index)                \
	{                                                         \
		...
		.modified = 1,                                        \
		.channel2 = _channel2,                                \
        .info_mask_separate = BIT(IIO_CHAN_INFO_RAW)|BIT(IIO_CHAN_INFO_CALIBBIAS)|BIT(IIO_CHAN_INFO_OFFSET),       \
        .info_mask_shared_by_type = BIT(IIO_CHAN_INFO_SCALE),   \
        .scan_index = _index,                                 \
		.scan_type  = IIO_ST('s', 16, 16, 0)                  \
	}
		...
		const struct i2c_device_id *id)
{
	...
	if (st->chip_config.has_compass) {
		...
		/* mpu65xx set to bypass mode */
		result = inv_i2c_single_write(st, REG_INT_PIN_CFG,
			st->plat_data.int_config | BIT_BYPASS_EN);
		if (result)
			return result;
		pr_info("mpu65xx bypass mode, %s\n", __func__);
	}
#endif
	...
	const struct i2c_device_id *id)
{
	...
	if (hw_id <= 4) {
		pr_info("imu sensor hardware reset\n");
		ret = gpio_request(P_VCIR_EN, "P_VCIR_EN");
		if (ret) {
			pr_err("gpio %d request failed\n", P_VCIR_EN);
			goto err_gpio;
		}

		ret = gpio_direction_output(P_VCIR_EN, 0);
		if (ret) {
			pr_err("gpio %d unavalialbe for output\n", P_VCIR_EN);
			goto err_free_gpio;
		}
		msleep(100);
		ret = gpio_direction_input(P_VCIR_EN);
		if (ret) {
			pr_err("gpio %d unavalialbe for input\n", P_VCIR_EN);
			goto err_free_gpio;
		}
		msleep(100);
		pr_info("GPIO pin requested ok, P_VCIR_EN = %s\n",
				gpio_get_value(P_VCIR_EN)?"H":"L");
	}
	...
	if (hw_id >= 4) {
		pr_info("=== Use gyro_platform_data_second ===");
		st->plat_data = gyro_platform_data_second;
	} else {
		pr_info("=== Use gyro_platform_data ===");
	...
	gpiod = devm_gpiod_get_index(dev, "inv_mup_int", 0);
	if (!IS_ERR(gpiod)) {
	...
	v3p3 = regulator_get(dev, "vmmc");
	tmp=regulator_enable(v3p3);
	if(tmp)
		pr_err("V3P3 regulator_enable failed\n");
...
#ifdef CONFIG_ACPI
static struct acpi_device_id mpu6500_acpi_match[] = {
	{ "INVN6500", 0 },
	{ "", 0 },
};
MODULE_DEVICE_TABLE(acpi, mpu6500_acpi_match);
#endif
...
printk(KERN_ERR "MPU65000 init success!\n");
	return 0;
}
