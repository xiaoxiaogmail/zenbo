...
#define AMP_SWITCH 332

extern int hs_get_status(void);
extern int hs_disable_detect(void);
extern int hs_enable_detect(void);
static int rt5647_codec_status;
...
static int amp_switch_init(void)
{
	int ret = 0;
	ret = gpio_request(AMP_SWITCH, "amp_switch");
	if (ret){
		printk(KERN_INFO"gpio %d request failed \n", AMP_SWITCH);
		goto err_gpio_amp;
	}
	ret = gpio_direction_output(AMP_SWITCH, 0);
	if (ret) {
		printk(KERN_INFO"gpio %d unavaliable for output \n", AMP_SWITCH);
		goto err_free_gpio_amp;
	}

	printk(KERN_INFO "GPIO pin requested ok, AMP_SWITCH = %s\n", gpio_get_value(AMP_SWITCH)? "H":"L");
	return 0;

err_free_gpio_amp:
	if (gpio_is_valid(AMP_SWITCH))
		gpio_free(AMP_SWITCH);
err_gpio_amp:
	return ret;
}
...
#if AMIC /* bard: No LOUT on this project */
...
#if 0 /* bard: No SPK on this project */
...
static const struct snd_kcontrol_new rt5647_snd_controls[] = {
	...
	/* DRC Limit Control */
	SOC_SINGLE("DRC Limit Control", RT5647_DRC1_HL_CTRL2,
		8, 7, 0),

	/* mx b3 */
	SOC_SINGLE("DRC Noise Gate Hold Time", RT5647_ALC_DRC_CTRL2, 12, 7, 0),
	SOC_SINGLE("DRC Pre Boost", RT5647_ALC_DRC_CTRL2, 6, 39, 0),
	SOC_SINGLE("DRC Right Post Boost", RT5647_ALC_DRC_CTRL2, 0, 63, 0),

	/* mx b4 */
	SOC_SINGLE("DRC Enable Control", RT5647_ALC_CTRL_1, 14, 3, 0),
	SOC_SINGLE("DRC Attack Rate", RT5647_ALC_CTRL_1, 8, 16, 0),
	SOC_SINGLE("DRC Sample Rate Control", RT5647_ALC_CTRL_1, 5, 7, 0),
	SOC_SINGLE("DRC Recovery Rate Control", RT5647_ALC_CTRL_1, 0, 16, 0),

	/* mx b5 */
	SOC_SINGLE("DRC Comp2 Ratio Selection", RT5647_ALC_CTRL_2, 14, 3, 0),
	SOC_SINGLE("DRC Left Post Boost", RT5647_ALC_CTRL_2, 8, 63, 0),
	SOC_SINGLE("DRC Compression Enable", RT5647_ALC_CTRL_2, 7, 1, 0),
	SOC_SINGLE("DRC Ratio Selection", RT5647_ALC_CTRL_2, 5, 3, 0),
	SOC_SINGLE("DRC Drop Enable", RT5647_ALC_CTRL_2, 4, 1, 0),
	SOC_SINGLE("DRC Noise Gate Ratio", RT5647_ALC_CTRL_2, 0, 3, 0),

	/* mx b6 */
	SOC_SINGLE("DRC Noise Gate Boost", RT5647_ALC_CTRL_3, 12, 0x0f, 0),
	SOC_SINGLE("DRC Threshold Min Control", RT5647_ALC_CTRL_3, 5, 0x2e, 0),
	SOC_SINGLE("DRC Noise Gate Threshold", RT5647_ALC_CTRL_3, 0, 0x1f, 0),

	/* mx b7 */
	SOC_SINGLE("DRC THMIN Fast Recover Switch", RT5647_ALC_CTRL_4, 14, 1, 0),
	SOC_SINGLE("DRC Noise Gate Switch", RT5647_ALC_CTRL_4, 13, 1, 0),
	SOC_SINGLE("DRC Threshold2 Max Control", RT5647_ALC_CTRL_4, 6, 0x3f, 0),
	SOC_SINGLE("DRC Threshold Max Control", RT5647_ALC_CTRL_4, 0, 0x3f, 0),

	/* mx e9 */
	SOC_SINGLE("DRC Bass Limiter Control", RT5647_DRC2_HL_CTRL2, 8, 7, 0),

	/* mx ea */
	SOC_SINGLE("MBDRC Bass Front Left Mute Control", RT5647_MUTI_DRC_CTRL1,
			11, 1, 1),
	SOC_SINGLE("MBDRC Bass Front Right Mute Control", RT5647_MUTI_DRC_CTRL1,
			10, 1, 1),
	SOC_SINGLE("MBDRC Bass Back Left Mute Control", RT5647_MUTI_DRC_CTRL1,
			9, 1, 1),
	SOC_SINGLE("MBDRC Bass Back Right Mute Control", RT5647_MUTI_DRC_CTRL1,
			8, 1, 1),
	SOC_SINGLE("MBDRC Front Mix Control", RT5647_MUTI_DRC_CTRL1,
			5, 1, 0),

	/* mx f0 */
	SOC_SINGLE("DRC Bass Noise Gate Hold Time Control",
				RT5647_DRC2_CTRL1, 12, 7, 0),
	SOC_SINGLE("DRC Bass Band Pre Boost Gain Control",
				RT5647_DRC2_CTRL1, 6, 39, 0),
	SOC_SINGLE("DRC Bass Band Post Boost Gain Right Control",
				RT5647_DRC2_CTRL1, 0, 63, 0),

	/* mx f1 */
	SOC_SINGLE("DRC Bass Band Attack Rate Control",
				RT5647_DRC2_CTRL2, 8, 31, 0),
	SOC_SINGLE("DRC Bass Band Sample Rate Control",
				RT5647_DRC2_CTRL2, 5, 7, 0),
	SOC_SINGLE("DRC Bass Band Recovery Rate Control",
				RT5647_DRC2_CTRL2, 0, 31, 0),

	/* mx f2 */
	SOC_SINGLE("DRC Bass Band Compression 2 Ratio Control",
				RT5647_DRC2_CTRL3, 14, 3, 0),
	SOC_SINGLE("DRC Bass Band Post Boost Gain Left  Control",
				RT5647_DRC2_CTRL3, 8, 63, 0),
	SOC_SINGLE("DRC Bass Band Power Control",
				RT5647_DRC2_CTRL3, 7, 1, 0),
	SOC_SINGLE("DRC Bass Band Compression 1 Ratio Control",
				RT5647_DRC2_CTRL3, 5, 3, 0),
	SOC_SINGLE("DRC Bass Noise Gate Drop Mode Control",
				RT5647_DRC2_CTRL3, 4, 1, 0),
	SOC_SINGLE("DRC Bass Noise Gate Expansion Ratio Control",
				RT5647_DRC2_CTRL3, 0, 3, 0),

	/* mx f3 */
	SOC_SINGLE("DRC Bass Band Noise Gate Compensation",
				RT5647_DRC2_CTRL4, 12, 15, 0),
	SOC_SINGLE("DRC Bass Band Threshold Level Control",
				RT5647_DRC2_CTRL4, 5, 46, 0),
	SOC_SINGLE("DRC Bass Band Noise Gate Threshold Control",
				RT5647_DRC2_CTRL4, 0, 31, 0),

	/* mx f4 */
	SOC_SINGLE("DRC Bass Band THMIN Fast Recovery Control",
				RT5647_DRC2_CTRL5, 14, 1, 0),
	SOC_SINGLE("DRC Bass Band Noise Gate Function Control",
				RT5647_DRC2_CTRL5, 13, 1, 0),
	SOC_SINGLE("DRC Bass Band Threshold2 Control",
				RT5647_DRC2_CTRL5, 6, 31, 0),
	SOC_SINGLE("DRC Bass Band Threshold1 Control",
				RT5647_DRC2_CTRL5, 0, 31, 0),
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMU:
		...
		rt5647_index_write(codec, 0x95, 0x7EC1);
		rt5647_index_write(codec, 0x96, 0xEF09);
		rt5647_index_write(codec, 0x97, 0x0EB6);
		rt5647_index_write(codec, 0x98, 0x7EC1);
		rt5647_index_write(codec, 0x99, 0xEF09);
		rt5647_index_write(codec, 0x9A, 0x0EB6);
		rt5647_index_write(codec, 0x9B, 0x003F);
		rt5647_index_write(codec, 0x9C, 0x0090);
		rt5647_index_write(codec, 0x9D, 0x7814);
		/* MXB3-B7, MXF0-F4, MXEA*/
		snd_soc_write(codec, RT5647_ALC_DRC_CTRL2, 0x001f);
		snd_soc_write(codec, RT5647_ALC_CTRL_2, 0x1f00);
		snd_soc_write(codec, RT5647_ALC_CTRL_3, 0x001f);
		snd_soc_write(codec, RT5647_ALC_CTRL_4, 0x4000);
		snd_soc_write(codec, RT5647_DRC2_CTRL1, 0x001f);
		snd_soc_write(codec, RT5647_DRC2_CTRL2, 0x022e);
		snd_soc_write(codec, RT5647_DRC2_CTRL3, 0x1f00);
		snd_soc_write(codec, RT5647_DRC2_CTRL4, 0x001f);
		snd_soc_write(codec, RT5647_DRC2_CTRL5, 0x4004);
		snd_soc_write(codec, RT5647_MUTI_DRC_CTRL1, 0x0c20);
		snd_soc_write(codec, RT5647_ALC_CTRL_1, 0x420c);
		rt5647_index_write(codec, 0x94, 0xCF00);
		snd_soc_update_bits(codec, RT5647_MONO_OUT,
				RT5647_L_MUTE, 0);
		msleep(5);
		gpio_set_value(AMP_SWITCH, 1);
		printk(KERN_INFO "POST PMU, AMP_SWITCH = %s\n", gpio_get_value(AMP_SWITCH)? "H":"L");
		break;

	case SND_SOC_DAPM_PRE_PMD:
		gpio_set_value(AMP_SWITCH, 0);
		printk(KERN_INFO "PRE PMD, AMP_SWITCH = %s\n", gpio_get_value(AMP_SWITCH)? "H":"L");
		msleep(5);
		snd_soc_update_bits(codec, RT5647_MONO_OUT,
			RT5647_L_MUTE, RT5647_L_MUTE);
		rt5647_index_write(codec, 0x94, 0x8700);
		/* MXB3-B7, MXF0-F4, MXEA back to default*/
		snd_soc_write(codec, RT5647_MUTI_DRC_CTRL1, 0x0f20);
		snd_soc_write(codec, RT5647_ALC_CTRL_1, 0x0206);
		snd_soc_write(codec, RT5647_ALC_DRC_CTRL2, 0x001f);
		snd_soc_write(codec, RT5647_ALC_CTRL_2, 0x1f00);
		snd_soc_write(codec, RT5647_ALC_CTRL_3, 0x0000);
		snd_soc_write(codec, RT5647_ALC_CTRL_4, 0x4000);
		snd_soc_write(codec, RT5647_DRC2_CTRL1, 0x001f);
		snd_soc_write(codec, RT5647_DRC2_CTRL2, 0x0206);
		snd_soc_write(codec, RT5647_DRC2_CTRL3, 0x1f00);
		snd_soc_write(codec, RT5647_DRC2_CTRL4, 0x0000);
		snd_soc_write(codec, RT5647_DRC2_CTRL5, 0x4000);
		rt5647_index_write(codec, 0x95, 0x7DC2);
		rt5647_index_write(codec, 0x96, 0xA178);
		rt5647_index_write(codec, 0x97, 0x5383);
		rt5647_index_write(codec, 0x98, 0x7DC2);
		rt5647_index_write(codec, 0x99, 0xA178);
		rt5647_index_write(codec, 0x9A, 0x5383);
		rt5647_index_write(codec, 0x9B, 0x003E);
		rt5647_index_write(codec, 0x9C, 0x02C1);
		rt5647_index_write(codec, 0x9D, 0xD37D);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		rt5647_update_eqmode(codec, EQ_CH_DACL, TOBY_SPK_L);
		break;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	switch (event) {
	case SND_SOC_DAPM_POST_PMU:
		rt5647_update_eqmode(codec, EQ_CH_DACR, TOBY_SPK_R);
		break;
...
#ifdef LOUT_ALWAYS_ON
	{ "LOUT amp", NULL, "Haptic Generator" },
	{ "LOUT amp", NULL, "I2S1" },
	{ "LOUT amp", NULL, "dac stereo1 filter" },
	{ "LOUT amp", NULL, "DAC L1 Power" },
	{ "LOUT amp", NULL, "DAC R1 Power" },
#endif
	{ "LOUTL", NULL, "LOUT amp" },
	{ "LOUTR", NULL, "LOUT amp" },
...
#ifdef AMIC
#else
	snd_soc_update_bits(codec, RT5647_BASE_BACK, 0x0007, 0x0004);
#endif
...
{
	unsigned int val = 0;
	val = gpio_get_value(UART_SWITCH_GPIO);
        ...
        struct device_attribute *attr, const char *buf, size_t count)
{
	int ret = 0;
	if (*(buf) == '1') {
		/*  WA avoid system failed to suspend by
		    switch jack to codec to clean interrupt state.
		*/
		gpio_set_value(UART_SWITCH_GPIO, 0);
		msleep(30);
		ret  = hs_disable_detect();
		if (ret)
			dev_err(dev, "disable headset detect failed\n");
		gpio_set_value(UART_SWITCH_GPIO, 1);
	} else if (*(buf) == '0') {
		gpio_set_value(UART_SWITCH_GPIO, 0);
		ret = hs_enable_detect();
		if (ret)
			dev_err(dev, "enable headset detect failed\n");
	}
...
static DEVICE_ATTR(rt5647_ctl_status, S_IRUGO, codec_ctl_status_show, NULL);
static DEVICE_ATTR(rt5647_uart_switch, S_IRUGO | S_IWUSR | S_IWGRP,
...
static int rt5647_probe(struct snd_soc_codec *codec)
{
...
#ifdef AMIC
	printk(KERN_INFO "Audio : Current support AMIC\n");
#else
	printk(KERN_INFO "Audio : Current support DMIC\n");
#endif

	ret = amp_switch_init();
	if (ret) {
		dev_err(codec->dev, "Request AMP_SWITCH Failed (%d)\n", ret);
	}

	pr_debug("Enter %s\n", __func__);
	pr_info("Codec driver version %s\n", VERSION);

	rt5647_codec_status = 0;
	audio_codec = NULL;
	...
	ret = device_create_file(codec->dev, &dev_attr_rt5647_ctl_status);
	if (ret != 0) {
		dev_err(codec->dev, "Failed to crate rt5647_ctl_status sysfs files %d\n", ret);
		return ret;
       }
	ret = device_create_file(codec->dev, &dev_attr_rt5647_uart_switch);
	if (ret != 0) {
	dev_err(codec->dev, "Failed to crate rt5647_uart_switch %d\n", ret);
	return ret;
	}

	if (snd_soc_read(codec, RT5647_VENDOR_ID1) == 0x10ec)
		rt5647_codec_status = 1;
	else
		pr_err("%s: incorrect audio codec rt5647 vendor ID\n", __func__);

	ret = gpio_request(UART_SWITCH_GPIO, "uart_jack_switch");

	if (!ret) {
		/* set phone jack default to UART */
		gpio_direction_output(UART_SWITCH_GPIO, 1);
	} else
	pr_err("%s: request gpio-%d failed", __func__, UART_SWITCH_GPIO);
...
static struct acpi_device_id rt5647_acpi_match[] = {
	{ "10EC5647", 0 },
	{ },
};
MODULE_DEVICE_TABLE(acpi, rt5647_acpi_match);

static const struct i2c_device_id rt5647_i2c_id[] = {
	{ "rt5647", 0 },
	{ "10EC5647:00", 0},
	{ "10EC5647", 0},
	{ "i2c-10EC5647:00:1b", 0},
	{ }
};
...
struct i2c_driver rt5647_i2c_driver = {
	.driver = {
		.name = "rt5647",
		.owner = THIS_MODULE,
		.acpi_match_table = ACPI_PTR(rt5647_acpi_match),
	},
