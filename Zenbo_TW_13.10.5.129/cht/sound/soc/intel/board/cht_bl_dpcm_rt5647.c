/*
 *  cht_bl_dpcm_rt5647.c - ASoc DPCM Machine driver
					...
					struct snd_soc_codec *codec)
{
	...
	status = rt5647_check_jd_status(codec);
	...
	if (!status) {
		...
		jack_type = rt5647_headset_detect(codec, true);
...
static int cht_hs_detection(void *data)
{
	...
	if (!jack->status) {
		 ...
		 */
/* Temporary workaround to remove JD and BP */
#if 0
		status = rt5647_check_jd_status(codec);
#endif
...
static void cht_check_hs_remove_status(struct work_struct *work)
{
	...
	if (jack->status) {
		...
		if (status) {
			...
			status = rt5647_headset_detect(codec, false);
			jack_type = 0;
...
static void cht_check_hs_button_status(struct work_struct *work)
{
	...
	if (((jack->status & SND_JACK_HEADSET) == SND_JACK_HEADSET)
		...
		if (!status) {
			...
			status = rt5647_check_bp_status(codec);
...
static inline struct snd_soc_codec *cht_get_codec(struct snd_soc_card *card)
{
	...
	list_for_each_entry(codec, &card->codec_dev_list, card_list) {
		if (!strstr(codec->name, "i2c-10EC5647:00")) {
		...
		struct snd_kcontrol *k, int  event)
{
	...
	if (SND_SOC_DAPM_EVENT_ON(event)) {
		...
		snd_soc_codec_set_sysclk(codec, RT5647_SCLK_S_PLL1,
		 ...
		 */
		snd_soc_codec_set_sysclk(codec, RT5647_SCLK_S_RCCLK,
...
//	struct snd_soc_codec *codec = codec_dai->codec;
	...
	if (strncmp(codec_dai->name, "rt5647-aif1", 11)) {
		pr_err("%s rt5647-aif1 codec_dai name is valid!\n", __func__);
		return 0;
	}
...
/* TDM Mode */
	...
	/* I2S 2 slot 24 bit set the Rx and Tx bitmask to
	 * 2 active slots as 0x3
	 */
	ret = snd_soc_dai_set_tdm_slot(codec_dai, 0x3, 0x3, 2,
			SNDRV_PCM_FORMAT_S24_LE);
	if (ret < 0) {
		pr_err("can't set codec I2S slot %d\n", ret);
		return ret;
	}

	/* RT5647 I2S slave Mode */
	fmt =   SND_SOC_DAIFMT_I2S | SND_SOC_DAIFMT_NB_NF
		| SND_SOC_DAIFMT_CBS_CFS;

	/* RT5647 I2S Mode 2ch 24 bits interface control */
	snd_soc_update_bits(codec, RT5647_TDM_CTRL_1, 0x7c00, 0x0800);
...
/* Temporary workaround to remove JD and BP */
#if 0
	snd_soc_update_bits(codec, RT5647_JD_CTRL,
			RT5647_JD_MASK, RT5647_JD_JD1_IN4P);
...
static struct snd_soc_dai_link cht_dailink[] = {
	...
	[CHT_DPCM_CAPTURE] = {
		.name = "Cherrytrail Capture Port",
		.stream_name = "Cherrytrail Capture",
...
{
	pr_info("Cherrytrail Machine Driver cht_rt5647 registerd\n");
...
MODULE_ALIAS("platform:cht_rt5647");
