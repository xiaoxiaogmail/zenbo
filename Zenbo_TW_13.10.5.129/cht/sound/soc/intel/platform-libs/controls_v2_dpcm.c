				 ...
				 void *cmd_data, u16 len)
{
	...
	pr_debug("DEBUG PRIV ->  %s\n length -> %d" , __func__,len);
...
void send_ssp_cmd(struct snd_soc_platform *platform, const char *id, bool enable)
{
	...
	pr_debug("%s: ssp_id: %u, mux: %d, domain: %d, \
	        nb_bits_per_slots %x, active_tx_slot_map %x, \
	        frame_sync_frequency %x, data_polarity %x, \
	        frame_sync_width %x, ssp_protocol %x\n", __func__,
		 config->ssp_id, mux, domain,
		 config->bits_per_slot, config->active_slot_map,
		 config->fs_frequency, config->data_polarity,
		 config->fs_width, config->ssp_protocol);
...
#if 0 /* Set RT5647 I2S configuration */
...
#ifdef AMIC /* Set RT5647 I2S configuration */
	if (cmd.selection == SSP_CODEC) {
		cmd.selection = SSP_CODEC;
		cmd.nb_bits_per_slots = 24;
		cmd.nb_slots = 2;
